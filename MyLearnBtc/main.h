...
//交易磁盘保存 实现== ！= tostring 初始化设置地址，print打印地址
class CDiskTxPos
{
public:
    //文件、区块位置、交易位置
...
// output's public key.
//
//交易输入，包含上一个交易输出点位置、公钥签名
//此结构主要是为了说明用于交易的比特币来源
class CTxIn
{
public:
    //上笔交易的输出、签名、序列
...
//
//输出交易，包含了交易的比特币数量、公钥和一些标志
//此结构主要标明交易时发送的比特币数量、接收方地址、标志
class CTxOut
{
public:
    int64 nValue;
...
//1.结构 1）输出点COutPoint 2）输入点 CIPoint 3）输入交易 CTXIn 4）输出交易 CTXOut 5）交易信息 CTransaction
//交易信息，在网络中广播，包含多个交易输入、交易输出
class CTransaction
{
public:
    int nVersion;//版本号
    ...
    /*
     满足有效交易的条件：
     1.输入（vin）、输出交易数组（vout）不为空
     2.交易的大小不能超过区块最大值
     3.交易的单个输出交易值（txout.nValue）必须在0-2100万
     4.不能双重支付，交易输入的上次交易输出点不能重复（txin.preveout）
     5.交易时挖矿产生（coinBase），则交易输入数组中的索引项的签名大小（vin[0].scriptSig.size()）必须在2-100
     如果交易是普通交易，则交易输入的上一次出点不鞥呢为空
     */
